...
void create_beamsprobevgg(FILE* fid)
{
    ...
    float_type theta = atof(pulseshape+strlen(SHAPE_BEAMSPROBEVGG)+1);
    ...
    float_type probe_ellipticity = load_namedfloat(fid, "PROBE_ELLIPTICITY");
            
    
    float_type zR    = real(WAVENUMBER0)*ro*ro/2;
    float_type zRe   = zR/probe_ellipticity/probe_ellipticity;
    ...
    float_type delta = 0;
    float_type fo; if (zR < 2*f) fo = f; else fo = zR*zR  /2/f*(1-sqrt(1-4*f*f/zR/zR));
    float_type fe; if (zRe< 2*f) fe = f; else fe = zRe*zRe/2/f*(1-sqrt(1-4*f*f/zRe/zRe));
    ...
    for (int cx=0; cx<N_X; cx++)
    {
     ...
     for (int i=0; i<Nbeams; i++)
     {
      ...
      if (i == Nbeams-1) R = sqrt((X*X*probe_ellipticity*probe_ellipticity + Y*Y))/ro;
      ...
      {
       float_type phase = (R*R*ro*ro*real(WAVENUMBER[cw])/2/fo)+X*tan(theta/2)*real(WAVENUMBER[cw]);
       if (i==Nbeams-1) phase = X*X*real(WAVENUMBER[cw])/2/fe + Y*Y*real(WAVENUMBER[cw])/2/fo + X*tan(theta/2)*real(WAVENUMBER[cw]);
