...
for k = K:(K + 250)
    for i = 1:N
        if k>nu(i)
            SIGMA(i) = SIGMA(i) + exp(-alfa(i)*(k-nu(i)))*dawson(sqrt(beta(i)*(k-nu(i))));
        end
    end
end
...
set(gca,'xlim',[1e-2 1e3], 'Ylim', [1e-120 1e25],'XGrid','on','YGrid','on');
...
save('ionization_rate_Ar_3900nm_1atm_s_minus1.dat','data','-ascII')
