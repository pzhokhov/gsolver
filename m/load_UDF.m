...
if (iscomplex==0) 
 if (float_size == 8) 
     ...
     zN_ = floor(length(D)/tN/xN/yN);
     D = reshape(D(1:zN_*tN*xN*yN), tN, xN, yN, zN_);
 else
     D = (fread(fid, tN*xN*yN*zN, 'float'));
     zN_ = floor(length(D)/tN/xN/yN);
     D = reshape(D(1:zN_*tN*xN*yN), tN, xN, yN, zN_);
 end;
else
 ...
 zN_ = floor(length(buf)/tN/xN/yN/2);
 D = reshape(complex(buf(1:2:2*zN_*tN*xN*yN),buf(2:2:2*zN_*tN*xN*yN)), tN, xN, yN, zN_);
end;
D = squeeze(D);
fclose(fid);
