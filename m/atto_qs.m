...
function [d,di, t, dn, E0] = atto_qs(cep, I, e12, d12, Q2, al)
...
parfor j=1:length(cep); 
 ...
 for i=1:Nt;
   ...
   Phi = (ones(Nt,1)*sum(weff.*Theta_t,1) - cumsum(weff.*(Theta_t))).*dt;
   R = (Theta_t.*sin(Phi)).*exp(-(t(i)-t)*gamma12);
   rhopp(i,:) = (sum(R.*(Edot*Omega),1));
 end; 
 
dij = cumsum(rhopp./weff,1).*dt.*(exp(-(t/max(t)/0.8).^10)*(al.*d12));
dn(:,j) = cumsum(rhopp./weff.*cE*Omega)*dt;
