function [A, Z, Nt, tmin, tmax,xnet, ynet] = load_dump(filemask)
A = [];
files = dir(filemask);
if isempty(files)
...
A = complex(zeros(Nt, length(xnet), length(ynet)));
...
if ((nodes_y(2)-nodes_y(1))*(nodes_x(2)-nodes_x(1))*Nt > 1e8)
    ...
    for nt = 1 : Nt;
        A(nt, nx, ny) = P(nt+(nx-nodes_x(1)-1)*Nt+(ny-nodes_y(1)-1)*Nt*(nodes_x(2)-nodes_x(1)+1));
    end;
    if (~isempty(hwait))
...
for n = 2 : length(files);
    ...
    if nx == 0 continue; end;
    for nt = 1 : Nt;
        A(nt, nx, ny) = P(nt, nx-nodes_x(1), ny-nodes_y(1));
    end;
    if (~isempty(hwait))
        waitbar((nx + ny*(nodes_x(2)-nodes_x(1)-1))/((nodes_x(2)-nodes_x(1)-1)*(nodes_y(2)-nodes_y(1)-1)), hwait);
    end;
