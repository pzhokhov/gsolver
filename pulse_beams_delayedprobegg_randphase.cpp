...
void create_beams_delayedprobegg_randphase(FILE* fid)
{
    ...
    float_type theta = atof(pulseshape+strlen(SHAPE_BEAMS_DELAYEDPROBE_RANDPHASE)+1);
    ...
    float_type phase0 = 0; 
    if (ISMASTER) phase0 = 2*M_PI*frand();
    MPI_Bcast(&phase0, 1, MPI_FLOAT_TYPE, 0, MPI_COMM_WORLD);
    ...
    for (int i=0; i<Nbeams; i++)
    {
     ...
     for (int cx=0; cx<N_X; cx++)
     {
      ...
      for (int ct=0; ct<N_T; ct++) 
      {
       ...
       if (i==Nbeams-2 || i==Nbeams-1) carrier_phase += phase0;
